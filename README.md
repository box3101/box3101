<img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=200&section=header&text=hi&nbsp;soon😊!&fontSize=50" />

[![Hits](https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fgithub.com%2Fbox3101&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=hits&edge_flat=false)](https://hits.seeyoufarm.com)

## About

UI개발자는 기획자, 디자이너 ,BackEnd 개발자와의 소통이 매우 중요합니다. 

**"대인관계 속 사람들과 의사소통이 원활하게 될 수 있는 점이 저의 가장 큰 장점입니다.**"

안녕하세요. 6년 경력의 웹 퍼블리셔 및 프론트엔드 개발자 이찬용입니다.  
저는 대인관계에서 항상 상대방의 입장을 고려하고, 서로의 의견을 존중하며 소통하는 것을 중요시 여깁니다.

저에게 있어 책임감과 성실성은 가장 중요한 가치입니다.  
한 아이의 아버지로서 가정에 대한 책임감을 갖고 있으며, 이는 제 직업적 태도에도 깊이 반영되어 있습니다.   
업무에서 문제가 발생했을 때, 저는 책임감을 가지고 적극적으로 해결에 나서며,   
항상 동료들과의 협업을 통해 더 나은 결과를 만들어내는 데 집중합니다.   
이러한 가정과 직장에서의 책임감은 제가 모든 업무에 최선을 다하게 하는 원동력이 되고 있습니다.   

저는 6년 이상의 경력을 가진 퍼블리셔 및 프론트엔드 개발자이며,  
다양한 웹 프로젝트를 수행하면서 다양한 경험을 쌓아왔습니다.

저는 HTML, CSS, JavaScript를 비롯한 다양한 웹 기술에 능숙하며,  
최근에는 React, Vue.js와 같은 현대적인 프레임워크를 활용한 프로젝트들을 수행해왔습니다.

제가 가진 가장 큰 장점은 기획자, 디자이너, 백엔드 개발자와의 원활한 소통 능력입니다.  
 UI 개발자로서 이러한 의사소통 능력은 프로젝트의 성공에 크게 기여한다고 생각하고 있습니다.  

최근엔 성과관리 OKR 솔루션(HR) 개발 프로젝트에서 SaaS 플랫폼을 구축했습니다.  
이 프로젝트에서는 React와 Ant Design을 활용하여 사용자 중심의 UI/UX를 개발했습니다.   
함수형 컴포넌트와 React Hooks를 사용하여 효율적인 상태 관리를 구현했으며, 컴포넌트 기반 아키텍처를 설계하고 구현했습니다.    
또한 Storybook을 활용하여 컴포넌트를 개발하고 문서화했습니다.     
이를 통해 재사용 가능한 컴포넌트 라이브러리를 구축하여 개발 생산성을 크게 향상시켰고,   
Storybook을 통한 시각적 테스트로 QA 시간을 단축시키는 성과를 거두었습니다.  

그리고 포켓컴퍼니 라는 회사에서 프리랜서로 자사 서비스 포트폴리오 및 랜딩 페이지 개발을 진행하였습니다.   
Vue.js를 활용하여 동적 UI를 개발하고, 컴포넌트 기반 아키텍처를 통해 효율적인 코드 관리를 실현했습니다.     
또한 전략적 브랜딩을 위한 웹 디자인을 구현하여 회사의 이미지를 효과적으로 전달했습니다.    

같은 회사에서 두 번째 프로젝트로 CRM 동영상 서비스의 프론트엔드를 개발했습니다.  
 Vue.js와 비디오 스트리밍 기술을 활용하여 포켓강의 및 교육용 동영상 스트리밍 서비스를 개발했습니다.   
싱글 페이지 애플리케이션(SPA)으로 구현하여 사용자 경험을 향상시켰으며, 사용자 친화적인 비디오 플랫폼 인터페이스를 구축했습니다.  

이전 프로젝트에서는 인천광역시에서 다양한 웹 프로젝트를 수행하였고,     

인천광역시 대표 웹사이트(영어, 중국어, 일본어), 온라인 통합 예약, 인천시립 박물관, 인천뉴딜, 인천광역시 국가안전 체험관 등의 웹사이트 운영 및 패밀리 사이트 구축을 진행했으며, 다양한 웹 프로젝트를 수행하면서, 웹 접근성, 호환성, 접속성, 편의성, 속도 등 다양한 측면에서 노력하였습니다.  

특히 웹 접근성 인증마크 취득을 통해 웹 접근성 준수에 대한 노력을 인정받았으며, 최신 웹 기술과 프레임워크를 활용하여 호환성과 접속성을 높이는 작업을 수행하였습니다.   

또한, 사용자 경험을 개선하기 위해 편의성과 속도 개선에도 많은 노력을 기울였습니다. 이러한 노력으로 인천광역시와 함께 더 나은 웹 서비스를 제공하고자 노력해왔습니다.   

또한, 부서 외주작업으로 ICT 표준 성능 성과 관리 시스템 관리자 화면, 김포공항 접근관리 시스템 관리자 화면, 그리고 GSOP 시스템 등 다양한 시스템을 개발하였습니다.  

저는 앞으로도 프론트엔드 개발 분야에서 더욱 전문적으로 성장해 나가고자 합니다.  
최신 웹 기술과 프레임워크에 대한 지속적인 학습과 개발 경험을 통해 더욱 높은 수준의 프론트엔드 개발 능력을 향상시키고자 합니다.   
특히 React, Vue 등의 프레임워크와 함께 TypeScript 등의 최신 웹 기술에 대한 지식을 더욱 깊이 있게 습득하고자 합니다.  

또한, 사용자 경험에 대한 이해와 미적 감각을 바탕으로 보다 직관적이고 세련된 UI/UX를 구현하는데 노력하겠습니다.  
끝으로, 항상 변화하는 웹 기술과 트렌드를 주시하며, 고객과 사용자에게 가치 있는 웹 서비스를 제공하기 위해 최선을 다할 것입니다.  

감사합니다.

<h3 dir="auto"><a id="user-content--technical-skills" class="anchor" aria-hidden="true" href="#-technical-skills"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a><g-emoji class="g-emoji" alias="hammer_and_wrench" fallback-src="https://github.githubassets.com/images/icons/emoji/unicode/1f6e0.png">🛠</g-emoji> &nbsp;Technical Skills</h3> 

<p dir="auto">
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-React-05122A?style=flat&logo=react">
<img src="https://img.shields.io/badge/-React-05122A?style=flat&logo=react" alt="React" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-Vue.js-4FC08D?style=flat&logo=vue.js">
<img src="https://img.shields.io/badge/-Vue.js-4FC08D?style=flat&logo=vue.js" alt="Vue.js" style="max-width: 100%;"></a>&nbsp;  
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-Node.js-05122A?style=flat&logo=node.js">
<img src="https://img.shields.io/badge/-Node.js-05122A?style=flat&logo=node.js" alt="Node.js" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-JavaScript-05122A?style=flat&logo=javascript">
<img src="https://img.shields.io/badge/-JavaScript-05122A?style=flat&logo=javascript" alt="JavaScript" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-HTML-05122A?style=flat&logo=html5">
<img src="https://img.shields.io/badge/-HTML-05122A?style=flat&logo=html5" alt="HTML" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-CSS-05122A?style=flat&logo=css3&logoColor=1572B6">
<img src="https://img.shields.io/badge/-CSS-05122A?style=flat&logo=css3&logoColor=1572B6" alt="CSS" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-GitHub-05122A?style=flat&logo=github">
<img src="https://img.shields.io/badge/-GitHub-05122A?style=flat&logo=github" alt="GitHub" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-Markdown-05122A?style=flat&logo=markdown">
<img src="https://img.shields.io/badge/-Markdown-05122A?style=flat&logo=markdown" alt="Markdown" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-Photoshop-05122A?style=flat&logo=adobe-photoshop">
<img src="https://img.shields.io/badge/-Photoshop-05122A?style=flat&logo=adobe-photoshop" alt="Photoshop" style="max-width: 100%;"></a>&nbsp;
<a target="_blank" rel="noopener noreferrer nofollow" href="https://img.shields.io/badge/-Figma-05122A?style=flat&logo=figma">
<img src="https://img.shields.io/badge/-Figma-05122A?style=flat&logo=figma" alt="Figma" style="max-width: 100%;"></a>&nbsp;
</p>


  <br>
</p>




## Skill Inventory
- 성과관리 OKR 솔루션 개발 (SaaS 플랫폼) - 2024.06~2024.08 (주)이퀄썸
- 자사서비스 포트폴리오 및 랜딩 페이지 개발 - 2024.02~2024.03 ㈜포켓컴퍼니
- 자사서비스 CRM 동영상 서비스 구현 프론트엔드 개발 - 2024.03~2024.05 ㈜포켓컴퍼니
- 인천광역시 통합 웹사이트 운영 및 구축	2021.01~2022.12	인천광역시   ㈜이즈파크
- 한국은행 성과관리시스템 어드민 구축 2023.08~2024.01 진행중 한국은행 ㈜이즈파크
- 삼성 SDI 2차 고도화(GSOP 시스템 개발)	2023.03~2023.08	삼성 SDI	㈜이즈파크
- 인천광역시 중구청평생교육포털 2022.12~2023.02 인천광역시중구청 ㈜이즈파크
- 삼성 SDI(GSOP 시스템 개발)	2022.09~2023.01	삼성 SDI	㈜이즈파크
- ICT 한국표준협회 성과관리 시스템	2022.07~ 11	한국표준협회	㈜이즈파크
- G4b 기원지업플러스 리뉴얼	2022.04~ 2022.07	중소벤처기업부	㈜아사달
- 경남테크노파크 항공우주센터 프론트화면 구축	2021.11~2022.03	경남테크노파크	㈜이즈파크
- 김포공항 출입관리시스템 어드민화면 구축	2021.09~2021.11	한국공항공사	㈜이즈파크
- ICT표준연과성과관리시스템 어드민화면 구축	2021.06~2021.08	한국정보통신기술협회	㈜이즈파크
- 포켓컴퍼니 사이트 리뉴얼	2020.12~2021.01	㈜포켓컴퍼니	㈜포켓컴퍼니
- 티오더 사이트 구축	2020.11~2020.12	㈜티오더	㈜포켓컴퍼니
- 스타일윙 앱 구축	2020.10~2020.11	㈜스타일윙	㈜포켓컴퍼니
- MBC 아카데미 사이버 평생교육원 사이트 구축	2020.06~2020.09	㈜바른스터디	㈜케이와이엔네트웍스
- MBC 아케데미 원격 교육원 웹 접근성작업	2020.08~2020.09	㈜바른스터디	㈜케이와이엔네트웍스
- WEEP 사이트 구축	2020.02~2020.12	㈜케이엘큐브	㈜케이와이엔네트웍스
- 메드포갈릭 모바일앱 리뉴얼	2019.12~2020.02	이랜드	㈜케이와이엔네트웍스
- 더 반찬 PASS 기획전 이벤트 페이지 2019.11~2019.12	동원홈푸드	㈜케이와이엔네트웍스
- GREL 웹사이트 구축	2019.03~2019.05	㈜GREL	㈜케이와이엔네트웍스
- 건강회복 알리미 앱 구축  2018.12~2019.03	㈜이비엠센터	㈜케이와이엔네트웍스
- JW메리어트 호텔 사이트 운영	2018.08~2018.11	메리어트	 ㈜이트라이브
- 바슈롬 사이트 운영	2018.08~2018.11	바슈롬	㈜이트라이브
- 농민신문 사이트 운영	2018.08~2018.11	농민신문사	㈜이트라이브
- 검단인포 사이트 운영	2018.08~2018.11	렉커사저	㈜이트라이브
- AHC몰 사이트 운영	2018.03~2018.07	카버코리아	㈜디지털다임
- 할리스 사이트 운영	2018.03~2018.07	케이지할리스에프앤비	㈜디지털다임
- 롯데 그룹 사이트 운영	2018.03~2018.07	롯데그룹	㈜디지털다임


인천광역시 대표 홈페이지 웹 접근성 인증마크 획득 - 2021.03  
인천광역시 일자리포털 웹 접근성 인증마크 휙득 - 2021.11  
인천광역시 국민안전체험관 웹 접근성 인증마크 휙득 - 2021.12  
인천광역시 대표 홈페이지 웹 접근성 인증마크 갱신 - 2022.03  
IFEZ 인천경제자유구역 웹 접근성 인증마크 갱신 - 2022.03  
문화포털 웹 접근성 인증마크 휙득 - 2023.03  
인천광역시 중구청 평생교육센터 웹 접근성 인증마크 휙득 2023.04  

## portfolio

**🔗 개인 포트폴리오 URL [https://box3101.github.io/cy-web](https://box3101.github.io/cy-web/)**

**🔗 개발 블로그 URL : [https://https://bit.ly/cyleeBlog](https://bit.ly/cyleeBlog)**

## 지원사업 빌드업 랜딩 페이지 및 포트폴리오 ##

이 프로젝트는 포켓컴퍼니의 지원사업 빌드업 전략을 위한 핵심 랜딩 페이지를 개발하는 것을 목표로 했습니다.  
사용자의 참여를 유도하고, 투자자 및 이해관계자에게 회사의 비전과 서비스를 효과적으로 소개하기 위해 설계된 페이지입니다. 

🔗 사이트 주소  [https://포켓컴퍼니.com/portfolio](https://포켓컴퍼니.com/portfolio)

🔗 사이트 주소  [https://포켓컴퍼니.com/pocket-html/](https://포켓컴퍼니.com/pocket-html/)  

🔗 사이트 주소  [https://포켓컴퍼니.com/pocket-html/invest-build.html/](https://포켓컴퍼니.com/pocket-html/invest-build.html/)

## 토이프로젝트 #1 : 동물 플래시 카드 만들기

이 프로젝트를 시작하게 된 주된 동기는 제가 리액트에 대한 학습 경험을 실제로 적용해보고 싶었던 열망 때문입니다.  
저에게는 아직 9개월밖에 안된 아이가 있는데, 아기를 위한 도움이 될만한 프로젝트를 고민하던 중 플래시 카드 개발 프로젝트를 시작하게 되었습니다.
  
강의와 독학을 통해 쌓아온 지식을 실제 웹 애플리케이션 개발에 적용하는 것이 이 프로젝트를 시작하게 된 중요한 계기였습니다.
  
리액트의 핵심 요소들인 컴포넌트 기반 설계, 상태 관리, 라이프사이클 메서드 등을 다양하게 활용하는 이 프로젝트는 제가 이론적으로 배운 내용들이 실제로 어떻게 작동하는지 체험해볼 수 있는 귀중한 기회가 되었습니다.

🔗 git 주소 : https://github.com/box3101/AnimalCards

🔗 사이트 주소 : https://box3101.github.io/AnimalCards/

## 인천광역시 중구청평생교육포털 (width Astro 1.0)

ASTRO는 신선한 바람을 불어넣은 새로운 프레임워크로, 2022년 1월에 첫 정식 버전인 1.0이 출시되었습니다.   

이를 사용함으로써, 마크업의 유지보수가 훨씬 용이해졌다는 사실을 경험했습니다. 또한, 과거의 Gulp 대신 새롭게 적용된 설정 방식은 그 자체로 큰 매력이었습니다.

그러나 완벽하게 만족스럽다고 말하긴 어렵습니다. CSS나 JS 파일이 한 파일로 묶여 minify 처리가 이루어지지 않는 점은 아쉬움으로 남았습니다.   
그럼에도 불구하고, 앞으로의 업데이트를 통해 이런 문제점들이 해결될 것이라는 기대감을 갖고 있습니다.  

ASTRO를 활용한 웹 프로젝트 경험을 통해, 그 효율성과 편리성을 직접 체험할 수 있었습니다. 이를 통해 제 개발 환경을 한 단계 더 향상시킬 수 있었던 것은 무엇보다 값진 경험이었습니다.

🔗 GIT URL [https://github.com/box3101/AstroProject](https://github.com/box3101/AstroProject)

🔗ㅊ

- 2023.03 정보통신 접근성 인증 프로젝트(품질마크 갱신)

## 인천광역시

인천광역시 공식 웹사이트 리뉴얼 프로젝트는 웹 접근성과 편의성을 중점으로 진행되었습니다.  
이 프로젝트를 통해 공공기관 웹사이트의 접근성을 향상시키는 데 중요한 역할을 수행했습니다.   
  
2021년 1월부터 2월까지 이루어진 이 프로젝트에서는 웹 접근성, 호환성, 편의성, 접속성 부분에서 최우수 평가를 받는 성과를 거두었습니다.
이를 통해 정보통신 접근성 인증 품질마크를 획득하였습니다.

또한, 웹사이트의 다양한 언어 버전 제작에도 참여하였습니다.
  
각각 2021년 5월에서 6월, 그리고 8월에서 10월 사이에 영문, 중문, 일문 버전의 인천광역시 공식 웹사이트를 제작하였습니다.    
이를 통해 다양한 국적의 이용자들이 편리하게 웹사이트를 이용할 수 있도록 하였습니다.  

또한 패밀리사이트를 중심으로 다양한 부서의 웹사이트 리뉴얼에도 참여하였습니다.  
이 과정에서 '인천 일자리 포털', '인천 국민 안전체험관', '인천광역시 시립 박물관' 등 다양한 분야의 웹사이트를 새롭게 디자인하고 구축하였습니다.  

이런 다양한 프로젝트를 진행하면서, 웹 접근성과 편의성, 그리고 다양한 언어 지원을 중요하게 생각하는 공공기관 웹사이트의 특성을 깊이 이해하게 되었습니다.   
  
이 경험은 앞으로의 웹 개발 프로젝트에도 큰 도움이 될 것입니다.

🔗 대표 홈페이지 리뉴얼 [https://www.incheon.go.kr/index](https://www.incheon.go.kr/index)

- 2021.01 ~ 2021.02
- 2021.03 정보통신 접근성 인증 프로젝트(품질마크 휙득)
- 2021.10 행정안전부 품질관리 수준평가 (웹 접근성, 웹 호환성, 웹 편의성, 웹 접속성 부분) 최우수 평가
- 2022.03 정보통신 접근성 인증 프로젝트(품질마크 갱신)

🔗 패밀리사이트- 인천 일자리포털 [https://www.incheon.go.kr/job/index](https://www.incheon.go.kr/job/index)

- 2021.08 ~ 2021.10

🔗 대표홈페이지 영문 [https://www.incheon.go.kr/en/index](https://www.incheon.go.kr/en/index)

🔗 대표홈페이지 중문 [https://www.incheon.go.kr/jp/index](https://www.incheon.go.kr/jp/index)

🔗 대표홈페이지 일문 [https://www.incheon.go.kr/cn/index](https://www.incheon.go.kr/jp/index) 

- 2021.05 ~ 2021.06

🔗 패밀리사이트- 인천 일자리포털
- 2021.09 정보통신 접근성 인증 프로젝트(품질마크 휙득)

- 2021.08 ~ 2021.10
🔗 패밀리사이트-인천 국민 안전체험관 [(https://www.incheon.go.kr/119safe/index](https://www.incheon.go.kr/119safe/index)

- 2021.10 ~ 2021.12
- 2021.12 정보통신 접근성 인증 프로젝트(품질마크 휙득)

🔗 패밀리사이트-인천광역시 시립 박물관 [https://www.incheon.go.kr/museum/index](https://www.incheon.go.kr/museum/index)

- 2021.08 ~ 2021.10


## G4b 기업지원플러스

- UI/UX 구현 : 디자이너로부터 전달받은 UI/UX 디자인을 기반으로 다양한 애니메이션 및 전환 효과를 구현했습니다.   
- 반응형 웹
- 웹 표준 및 접근성 준수
- 크로스 브라우징 :다양한 브라우저와 플랫폼에서 웹사이트가 올바르게 동작하도록 테스트하고 최적화하였습니다.   
- 퍼포먼스 최적화 : 웹페이지 로딩 시간 최소화를 위한 이미지 최적화 및 코드 최적화 작업을 진행하였습니다.   


🔗 운영 페이지 [https://www.g4b.go.kr:441/svc/main.do](https://www.g4b.go.kr:441/svc/main.do)

🔗 가이드  [https://box3101.github.io/g4b/assets/guide/guide.html](https://box3101.github.io/g4b/assets/guide/guide.html)

## ****WEEP****

- UI/UX 구현: 사용자 친화적인 인터페이스로, 기업과 구직자 간의 원활한 연결을 도와주는 디자인을 구현하였습니다.
- 반응형 웹: 다양한 디바이스에서도 원활한 사용 경험을 제공하기 위해 반응형 웹 디자인을 적용하였습니다.
- 웹 표준 및 접근성 준수: 모든 사용자가 불편함 없이 사이트를 이용할 수 있도록 웹 표준 및 접근성을 최적화하였습니다.
- 크로스 브라우징: 다양한 브라우저에서도 사이트가 올바르게 동작하도록 테스트 및 최적화를 진행하였습니다.
- 퍼포먼스 최적화: 빠른 웹페이지 로딩과 원활한 중계 서비스를 제공하기 위해 코드 및 데이터베이스 성능을 최적화하였습니다.

🔗 작업 홈페이지  [http://weep2020.cafe24.com/index.html](http://weep2020.cafe24.com/index.html)

🔗 프론트 가이드  [http://weep2020.cafe24.com/weep0514.html](http://weep2020.cafe24.com/weep0514.html)


## ICT 한국표준협회 성과관리 시스템

- UI/UX 구현 : 전문 디자이너와의 협업을 통해 개발된 UI/UX 디자인에 따라 웹페이지의 효과적인 애니메이션 및 전환 효과를 성공적으로 구현하였습니다.
- 반응형 웹
- 웹 표준 및 접근성 준수
- 크로스 브라우징 : Internet Explorer, Chrome, Firefox, Safari 등 다양한 브라우저에서 웹사이트의 일관된 동작과 표현을 보장하기 위한 테스트 및 조정을 진행하였습니다.
- 퍼포먼스 최적화 : 사용자들에게 빠른 웹페이지 로딩 경험을 제공하기 위해, 이미지 및 코드의 최적화 작업을 수행하였습니다.

🔗 작업 홈페이지 [https://box3101.github.io/Association/assets/html/index.html](https://box3101.github.io/Association/assets/html/index.html)

🔗 가이드 페이지  (with notion) [ICT 한국표준협회(Front) 퍼블리싱 작업 현황](https://www.notion.so/ICT-Front-6022ab91223440fa88874b6f4bf7fb1a)

## ****경남테크노파크 항공우주 시험평가 지원 시스템****

- UI/UX 구현 : 전문적으로 설계된 UI/UX 디자인을 참조하여, 항공우주에 특화된 시험평가 지원 인터페이스의 애니메이션 및 전환 효과를 효과적으로 구현하였습니다.
- 반응형 웹 : 항공우주 전문가 및 연구원들이 다양한 디바이스에서 원활하게 시스템을 사용할 수 있도록 반응형 웹 디자인을 적용하였습니다.
- 웹 표준 및 접근성 준수 : 국제 항공우주 웹 표준 및 접근성 지침을 엄격히 준수하여, 모든 사용자가 웹사이트를 안전하고 효과적으로 이용할 수 있게 만들었습니다.
- 크로스 브라우징 : 항공우주 분야에서 주로 사용되는 브라우저 환경에서의 웹사이트의 안정성을 확보하기 위해 꼼꼼한 테스트 및 조정 작업을 수행하였습니다.
- 퍼포먼스 최적화 : 항공우주 데이터의 복잡성을 고려하여, 빠른 로딩 속도를 유지하면서 데이터 처리 효율성을 최적화하였습니다.

🔗 작업 홈페이지  [https://box3101.github.io/Gyeongnam/assets/html/main.html](https://box3101.github.io/Gyeongnam/assets/html/main.html)

🔗 프론트 가이드  [https://box3101.github.io/Gyeongnam/assets/guide/guide.html](https://box3101.github.io/Gyeongnam/assets/guide/guide.html)


## ****ICT표준연과성과관리시스템****

- UI/UX 구현: ICT 표준과 관련된 다양한 데이터를 효과적으로 표시하고 관리할 수 있는 사용자 친화적인 인터페이스를 구현하였습니다.
- 반응형 웹: 모든 디바이스에서 안정적인 성능을 보이며, 사용자에게 효과적인 화면을 제공하기 위해 반응형 웹 디자인을 적용하였습니다.
- 웹 표준 및 접근성 준수: ICT 표준연과성과 관련된 중요 정보를 모든 사용자가 불편함 없이 접근하고 사용할 수 있도록 웹 표준 및 접근성을 최적화하였습니다.
- 크로스 브라우징: 다양한 웹 브라우저에서도 시스템이 올바르게 동작하도록 테스트 및 최적화 작업을 진행하였습니다.
- 퍼포먼스 최적화: 빠른 응답 시간과 효율적인 데이터 처리를 위해 성능 최적화 작업을 수행하였습니다.

🔗 작업 홈페이지  [https://box3101.github.io/TTA/assets/html/main.html](https://box3101.github.io/TTA/assets/html/main.html)

🔗 프론트 가이드  [https://box3101.github.io/TTA/assets/guide/guide.html](https://box3101.github.io/TTA/assets/guide/guide.html)


**box3101/box3101** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
